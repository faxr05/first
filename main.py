...
# REFERAL TIZIMI O'CHIRILGAN VERSIYA
# """
...
#     data = get_user_data(user_id)
#     return data['used_today'] < data['limit']

# def use_generation(user_id: int):
#     data = get_user_data(user_id)
#     data['used_today'] += 1

# def get_remaining_limit(user_id: int) -> tuple:
#     check_and_reset_daily_limit(user_id)
#     data = get_user_data(user_id)
#     return (data['limit'] - data['used_today'], data['limit'])
...
#     remaining, total = get_remaining_limit(user_id)
...
#     get_user_data(callback.from_user.id)['language'] = lang
